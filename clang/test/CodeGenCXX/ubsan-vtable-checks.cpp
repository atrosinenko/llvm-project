...
int get_v(T* t) {
  ...
  // CHECK-PTRAUTH: [[STRIPPED_VTABLE:%.*]] = call ptr @llvm.ptrauth.strip.p0(ptr %vtable) [ "ptrauth"(i64 0) ], !nosanitize
  // CHECK-PTRAUTH: [[STRIPPED_INT:%.*]] = ptrtoint ptr [[STRIPPED_VTABLE]] to i64
  ...
  // CHECK-PTRAUTH: [[AUTHED_PTR:%.*]] = call ptr @llvm.ptrauth.auth.p0(ptr %vtable2) [ "ptrauth"(i64 2, i64 17113, i64 {{%.*}}) ]
...
void delete_it(T *t) {
  ...
  // CHECK-PTRAUTH: [[STRIPPED_VTABLE:%.*]] = call ptr @llvm.ptrauth.strip.p0(ptr [[VTABLE:%.*]]) [ "ptrauth"(i64 0) ]
  // CHECK-PTRAUTH: [[STRIPPED_INT:%.*]] = ptrtoint ptr [[STRIPPED_VTABLE]] to i64
  // CHECK-PTRAUTH: {{%.*}} = mul i64 [[STRIPPED_INT]], {{.*}}
...
U* dyncast(T *t) {
  ...
  // CHECK-PTRAUTH: [[STRIPPED_VTABLE:%.*]] = call ptr @llvm.ptrauth.strip.p0(ptr {{.*}}) [ "ptrauth"(i64 0) ]
  // CHECK-PTRAUTH: [[STRIPPED_INT:%.*]] = ptrtoint ptr [[STRIPPED_VTABLE]] to i64
  // CHECK-PTRAUTH: {{%.*}} = mul i64 [[STRIPPED_INT]], {{.*}}
  // CHECK-VPTR: call void @__ubsan_handle_dynamic_type_cache_miss_abort
  // CHECK-PTRAUTH: [[AUTHED_PTR:%.*]] = call ptr @llvm.ptrauth.auth.p0(ptr %vtable1) [ "ptrauth"(i64 2, i64 17113, i64 {{%.*}}) ]
