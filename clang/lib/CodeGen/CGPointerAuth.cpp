...
CodeGenFunction::emitPointerAuthResignCall(llvm::Value *Value,
  ...
  llvm::OperandBundleDef CurSchema("ptrauth",
                                   ArrayRef({CurKey, CurDiscriminator}));
  llvm::OperandBundleDef NewSchema("ptrauth",
                                   ArrayRef({NewKey, NewDiscriminator}));

  // call i64 @llvm.ptrauth.resign(i64 %pointer) [ "ptrauth"(<cur_schema>),
  //                                               "ptrauth"(<new_schema>) ]
  auto *Intrinsic = CGM.getIntrinsic(llvm::Intrinsic::ptrauth_resign);
  Value = EmitRuntimeCall(Intrinsic, {Value}, {CurSchema, NewSchema});
