...
public:
  ...
  // must be safe, unless it is AddrDisc that was passed-through unmodified.
  //
...
private:
  ...
  void emitBLRA(bool IsCall, AArch64PACKey::ID Key, Register Target, Register Disc);
...
void AArch64AsmPrinter::emitAUT(AArch64PACKey::ID Key, Register Pointer, Register Disc) {
...
void AArch64AsmPrinter::emitPAC(AArch64PACKey::ID Key, Register Pointer, Register Disc) {
  bool IsZeroDisc = Disc == AArch64::XZR;
...
void AArch64AsmPrinter::emitBLRA(bool IsCall, AArch64PACKey::ID Key, Register Target, Register Disc) {
  bool IsZeroDisc = Disc == AArch64::XZR;
...
Register AArch64AsmPrinter::emitPtrauthDiscriminator(uint64_t Disc,
  ...
  if (MayUseAddrAsScratch && isPtrauthRegSafe(AddrDisc)) {
    ...
    assert(ScratchReg != AddrDisc && "Must not clobber AddrDisc, but have to");
...
/// authenticated successfully. On success, execution continues on the next
